...
describe('impact parity', () => {
  it('reproduces the demo business month figures', () => {
    expect(repo.impactFor('biz_adaeze_frozen', 'month')).toEqual({
      litresDisplaced: 419,
      co2KgAvoided: 967.9,
  ...
  it('reproduces the demo business year figures', () => {
    expect(repo.impactFor('biz_adaeze_frozen', 'year')).toEqual({
  ...
  it('reproduces the demo business all-time figures', () => {
    expect(repo.impactFor('biz_adaeze_frozen', 'all')).toEqual({
  ...
  it('a business with no asset reports zero generation and ownership', () => {
    expect(repo.impactFor('biz_bilikisu_tailor', 'month')).toEqual({
      litresDisplaced: 158,
      co2KgAvoided: 365,
  ...
  it('reproduces a financed non-demo business year figures', () => {
    expect(repo.impactFor('biz_gwarinpa_mart', 'year')).toEqual({
  ...
  it('the wrapped report projects the same yearly impact', () => {
    const wrapped = computeWrapped({
      year: 2025,
      impact: repo.impactFor('biz_adaeze_frozen', 'year'),
      now: repo.now(),
    });
